...
#if COMPILER(MSVC) && !OS(WINCE) && !OS(BLACKBERRY)
#include <crtdbg.h>
#include <mmsystem.h>
#include <windows.h>
#endif
...
EncodedJSValue JSC_HOST_CALL functionAlert(ExecState* exec)
{
    if (jscClient) {
        if (exec->argumentCount()) {
            ...
            jscClient->alert(message.characters(), message.length());
        } else
            jscClient->alert(0, 0);
    }
    return jsNull();
...
EncodedJSValue JSC_HOST_CALL functionConfirm(ExecState* exec)
{
    ...
    if (jscClient) {
        if (exec->argumentCount()) {
            UString message = exec->argument(0).toString(exec);
            returnValue = jscClient->confirm(message.characters(), message.length());
        } else
...
EncodedJSValue JSC_HOST_CALL functionPrompt(ExecState* exec)
{
    ...
    if (jscClient) {
        ...
        if (!numArgs) {
            ...
            UString message = exec->argument(0).toString(exec);
            if (numArgs == 1) {
                jscClient->prompt(message.characters(), message.length(), 0, 0, &callback);
            } else {
                UString defaultValue = exec->argument(1).toString(exec);
                jscClient->prompt(message.characters(), message.length(), defaultValue.characters(), defaultValue.length(), &callback);
            }
        }
    }

...
static JSCell* formatLocaleDate(ExecState* exec, DateInstance* dateObject, double timeInMilliseconds, LocaleDateTimeFormat format)
{
    ...
    UDateFormat* df = udat_open(timeStyle, dateStyle, NULL, 0, -1, NULL, 0, &status);
    if (!df)
        return jsEmptyString(exec);

    UChar buffer[128];
    int32_t length;
    length = udat_format(df, timeInMilliseconds, buffer, 128, NULL, &status);
    udat_close(df);

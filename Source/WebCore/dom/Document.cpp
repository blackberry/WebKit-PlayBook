...
void Document::processHandheldFriendly(const String& features)
{
    ...
    if (frame->page()
        ...
        && equalIgnoringCase(features, "true")
        && m_viewportArguments == ViewportArguments()) {    // Only if no viewport set will we proceed with HandHeldFriendly

        ViewportArguments arguments;
        processArguments("initial-scale=1.0", (void*)&arguments, &setViewportFeature);
        frame->page()->chrome()->client()->dispatchViewportPropertiesDidChange(arguments);
    }
}
#endif

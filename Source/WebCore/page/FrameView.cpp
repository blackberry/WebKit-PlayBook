...
void FrameView::setFrameRect(const IntRect& newRect)
{
    ...
    updateScrollableAreaPageMap();
}

void FrameView::updateScrollableAreaPageMap()
{
    // Check for:
    // 1) display:none set to self or inherited.
    // 2) visibility:hidden set to self or parent;
    // 3) overflow{-x,-y}: hidden;
    // 4) scrolling: no;
    // 5) Frame whose area is 1. Inner frames whose width=height=frameborder=0 still
    //    produce a 1x1 sized frame.
    IntSize contentSize = contentsSize();
    IntSize visibleContentSize = visibleContentRect().size();

    // 'display: none' causes the visible content size to be 0x0 (covers #1).
    if ((contentSize.height() > visibleContentSize.height() || contentSize.width() > visibleContentSize.width())
        && (visibleContentSize.height() * visibleContentSize.width() > 1)
        && (contentSize.height() * contentSize.width() > 1)) {

        // Covers #2 and #5.
        HTMLFrameOwnerElement* owner = m_frame->ownerElement();
        if (!owner || !owner->renderer() || !owner->renderer()->visibleToHitTesting())
            goto removeScrollable;

        ScrollbarMode horizontalMode;
        ScrollbarMode verticalMode;
        calculateScrollbarModesForLayout(horizontalMode, verticalMode, RulesFromWebContentOnly); // Covers #3 and #4.
...
void FrameView::setContentsSize(const IntSize& size)
{
    ...
    page->chrome()->contentsSizeChanged(frame(), size_); //notify only
...
void FrameView::checkRelayoutForContentsSizeFromHistory()
{
    ...
    // schedule a relayout
    if (!m_contentsSizeFromHistory.isEmpty() && m_contentsSizeFromHistory != IntSize(root->maxXLayoutOverflow(), root->maxYLayoutOverflow()))
        scheduleRelayout();
}
#endif
...
void FrameView::adjustViewSize()
{
    ...
    bool overflowExceedsContentsSize = root->rightAbsoluteVisibleOverflow() > root->maxXLayoutOverflow() ||
                                       root->bottomAbsoluteVisibleOverflow() > root->maxYLayoutOverflow();

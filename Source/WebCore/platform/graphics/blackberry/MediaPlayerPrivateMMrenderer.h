...
public:
    MediaPlayerCallbackData(void* mp, const char* flag, const char* value)
     : m_mp(mp), m_flag(0), m_value(0), m_floatval(0)
    {
        m_flag = static_cast<char*>(malloc(strlen(flag)+1));
        strcpy(m_flag, flag);
        if (value) {
...
class IntSize;
class IntRect;
class ConcurrentMediaSet;

class MediaPlayerPrivate : public MediaPlayerPrivateInterface, BlackBerry::Platform::MediaConnListener {

    friend int ppsHandler(const char* name, pps_obj_event_t, const strm_dict_t*, void* mp);
    friend void* workerThread(void* mp);
    friend void mpOutputUpdate(const BlackBerry::Platform::Graphics::Window*);
    friend void mpCallbackHandler(void* data);
    friend void mpResizeSourceDimensions(void* data);
    friend class ConcurrentMediaSet;
    ...
    public:
        ...
        // Volume goes from 0.0 (quietest) to 1.0 (loudest).
        ...
        int showErrorDialog(BlackBerry::WebKit::WebPageClient::AlertType atype) const;

        static void notifyAppActivatedEvent(bool activated);
        static void setCertificatePath(const String& caPath) { s_caPath = caPath; }

        unsigned sourceWidth() { return m_sourceWidth; }
        unsigned sourceHeight() { return m_sourceHeight; }
    ...
    private:
        ...
        const char* stateStr(MpState) const;

        int volumeCallbackMsgSend(const char* flags, const char* value, float newVolume);
        int mediaStateCallbackMsgSend(const char* flag, const char* value);
        ...
        int playStateHandle(pps_obj_event_t, const strm_dict_t*);
        int playStatusHandle(pps_obj_event_t, const strm_dict_t*);
        int playOutputHandle(pps_obj_event_t, const strm_dict_t*);
        int metadataHandle(pps_obj_event_t, const strm_dict_t*);
        int audioStatusHandle(pps_obj_event_t, const strm_dict_t*);
        ...
        void volumeCallbackHandler(const char* flag, float newVolume);
        int mediaStateChangedProcessing(uint32_t flags);
        void mediaStateCallbackHandler(const char* flag, const char* value);
        void metadataCallbackHandler(const char* flag, const char* value);
        ...
        bool metadataTerminate(mmmd_hdl_t*);
        int metadataGet(const String& url, mmmd_hdl_t*);
        int metadataParse(char* md);

        void signalMetadataCondvarFinished();

        static ConcurrentMediaSet s_activeElements;
        ...
        void outputUpdate(const IntRect&);

        BlackBerry::Platform::Graphics::Window* getPeerWindow(const char* uniqueId) const;
        bool mmrPaused() const;
        ...
        virtual void onMediaStateChange(BlackBerry::Platform::MediaConnListener::MState state) { }; // Will never be called
    ...
    private:
        ...
        const char* mmrPlaystateStr(MMRPlayState) const;
        ...
        BlackBerry::Platform::Graphics::Window* m_parentWindow;
        ...
        BlackBerry::Platform::Graphics::Window* screenWindowSet();
        void windowCleanup();
        ...
        void setVolumeFromDict(const strm_dict_t*);
        ...
        BlackBerry::Platform::MediaConn* m_mediaConn;

        bool m_allowPPSVolumeUpdates;
        uint64_t m_lastSystemVolumeChangeTime;
        uint64_t m_lastControlVolumeChangeTime;
        uint32_t m_lastSeekPosition;
        int m_skipPPSPositionUpdates;
        ...
        BlackBerry::WebKit::WebPageClient::AlertType m_pendingErrorDialog;
...
// A class to keep track of the set of media elements that is allowed to play
// concurrently. This can include at most one video element and any number
// of audio elements.
class ConcurrentMediaSet {
public:
    ConcurrentMediaSet()
        : m_videoCount(0), m_mediaCount(0), m_videoElement(0)
    {
    }
    ...
    void add(MediaPlayerPrivate* element)
    {
    ...
    void remove(MediaPlayerPrivate* element)
    {
        if (element && m_elements.contains(element)) {
    ...
    bool contains(MediaPlayerPrivate* element)
    {
        return m_elements.contains(element);
    }

    void stop(MediaPlayerPrivate* exclude)
    {
        HashSet<MediaPlayerPrivate*>::iterator end = m_elements.end();
        for (HashSet<MediaPlayerPrivate*>::iterator it = m_elements.begin(); it != end; ++it) {
            if (*it != exclude)
                (*it)->stop();
        }
    }
    ...
    void pause()
    {
        HashSet<MediaPlayerPrivate*>::iterator end = m_elements.end();
        for (HashSet<MediaPlayerPrivate*>::iterator it = m_elements.begin(); it != end; ++it) {
            HTMLMediaElement* element = static_cast<HTMLMediaElement*>((*it)->m_player->mediaPlayerClient());
            element->pause();
        }
    }
    ...
    MediaPlayerPrivate* videoElement()
    {
...
private:
    int m_videoCount;
    int m_mediaCount;
    MediaPlayerPrivate* m_videoElement;
    HashSet<MediaPlayerPrivate*> m_elements;
};

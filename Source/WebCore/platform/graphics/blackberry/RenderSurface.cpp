...
RenderSurface::RenderSurface(LayerRenderer* renderer, LayerCompositingThread* owner)
    : m_ownerLayer(owner)
...
void RenderSurface::setContentRect(const IntRect& contentRect)
{
    m_contentRect = contentRect;
    m_size = contentRect.size();
}

FloatRect RenderSurface::drawRect() const
{
...
bool RenderSurface::ensureTexture()
{
    if (!m_texture)
        m_texture = textureCacheCompositingThread()->createTexture(0);

    return m_texture->protect(m_size);
}

void RenderSurface::releaseTexture()
{
    m_texture->unprotect();
}

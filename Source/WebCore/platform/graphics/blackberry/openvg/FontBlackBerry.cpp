...
static String setupTextDrawing(const Font* font, const TextRun& run, BlackBerry::Platform::Text::DrawParam* drawParam)
{
    const float scaleFactor = font->primaryFont()->platformData().scaleFactor();

    drawParam->m_textOrder = run.rtl()
        ? BlackBerry::Platform::Text::ReverseTextOrder
        : BlackBerry::Platform::Text::NoTextOrder;
    drawParam->m_letterspace.m_value = font->letterSpacing() / scaleFactor;

    if (font->wordSpacing()) {
        drawParam->m_wordspace.m_mode = BlackBerry::Platform::Text::WordSpaceAdd;
    ...
    if (run.padding()) {
        ...
        if (gapCount) {
            ...
            drawParam->m_wordspace.m_mode = BlackBerry::Platform::Text::WordSpaceNatural;
    }
...
void Font::drawComplexText(GraphicsContext* context, const TextRun& run, const FloatPoint& point, int from, int to) const
{
    ...
    BlackBerry::Platform::Text::Font* font = fontPlatformData.font(); // FIXME: use fallback fonts as well?
    BlackBerry::Platform::Text::DrawParam drawParam;
    ...
    BlackBerry::Platform::Text::GraphicsContext* textContext = FontPlatformData::textGraphicsContext();

#if USE(EGL)
    textContext->setDisplay(static_cast<BlackBerry::Platform::Text::NativeGraphicsDisplay>(surface->eglDisplay()));
    textContext->setSurface(static_cast<BlackBerry::Platform::Text::NativeGraphicsSurface>(surface->eglSurface()));
    textContext->setContext(static_cast<BlackBerry::Platform::Text::NativeGraphicsContext>(surface->eglContext()));
    ASSERT_EGL_NO_ERROR();
#endif
    ...
    textContext->setDisplay(static_cast<BlackBerry::Platform::Text::NativeGraphicsDisplay>(sharedSurface->eglDisplay()));
    textContext->setSurface(static_cast<BlackBerry::Platform::Text::NativeGraphicsSurface>(sharedSurface->eglSurface()));
    textContext->setContext(static_cast<BlackBerry::Platform::Text::NativeGraphicsContext>(sharedSurface->eglContext()));
    ASSERT_EGL_NO_ERROR();
#endif
...
float Font::floatWidthForComplexText(const TextRun& run, HashSet<const SimpleFontData*>* fallbackFonts, GlyphOverflow*) const
{
    ...
    const FontPlatformData& fontPlatformData = primaryFont()->platformData();
    const float scaleFactor = fontPlatformData.scaleFactor();
    BlackBerry::Platform::Text::Font* font = fontPlatformData.font();
    BlackBerry::Platform::Text::DrawParam drawParam;
...
int Font::offsetForPositionForComplexText(const TextRun& run, float position, bool includePartialGlyphs) const
{
    ...
    FontPlatformData::engine()->xToTextPos(position / scaleFactor, offset,
        BlackBerry::Platform::Text::TextPosRoundNearest, *font,
        sanitized.characters(), sanitized.length(), drawParam);

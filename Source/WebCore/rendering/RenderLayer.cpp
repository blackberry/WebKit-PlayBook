...
bool RenderLayer::scrollsOverflow() const
{
    ...
    // XXX: parent() checking is temporary. scrollOverflow will be removed in the upstream by https://bugs.webkit.org/show_bug.cgi?id=79611.
    return box->parent() && box->hasOverflowClip() && box->scrollsOverflow() && (box->scrollHeight() != box->clientHeight() || box->scrollWidth() != box->clientWidth());
}
...
void RenderLayer::renderBoxGeometryChanged()
{
    updateScrollableAreasMap();
}

void RenderLayer::updateScrollableAreasMap()
{
    Frame* frame = renderer()->frame();
    if (!frame)
        return;

...
ResourceHolderImpl* ResourceHolderImpl::create(const WebCore::CachedResource& cachedResource)
{
    ...
    instance->m_url = cachedResource.url().string().utf8().data();
    instance->m_mimeType = cachedResource.response().mimeType().utf8().data();
    instance->m_suggestedFileName = cachedResource.response().suggestedFilename().utf8().data();

...
InRegionScroller::InRegionScroller(WebPagePrivate* webPage, RenderLayer* layer)
    : m_webPage(webPage)
    , m_layer(layer)
{
    ASSERT(webPage);
...
Platform::IntPoint InRegionScroller::calculateMinimumScrollPosition(const Platform::IntSize& viewportSize, float overscrollLimitFactor) const
{
...
Platform::IntPoint InRegionScroller::calculateMaximumScrollPosition(const Platform::IntSize& viewportSize, const Platform::IntSize& contentsSize, float overscrollLimitFactor) const
{
    // FIXME: Eventually we should support overscroll like iOS5 does.
    ASSERT(!allowsOverscroll());
...
RenderLayer* InRegionScroller::layer() const
{
    ASSERT(!m_isNull);
    return m_layer;
}

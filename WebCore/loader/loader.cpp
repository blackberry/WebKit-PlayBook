...
void Loader::servePendingRequests(Priority minimumPriority)
{
    ...
    Vector<RefPtr<Host> > hostsToServe;
    m_hosts.checkConsistency();
    ...
    for (unsigned n = 0; n < hostsToServe.size(); ++n) {
        Host* host = hostsToServe[n].get();
        // Loader::servePendingRequests() can be re-entered for example when JS dialog is
        // showing and the subsequent run may have removed the host.
        if (host->hasOneRef())
            continue;
        if (host->hasRequests())

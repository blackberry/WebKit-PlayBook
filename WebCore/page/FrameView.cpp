    ...
    if (settings) {
        ...
        if (node && node->renderer()) {
            ...
            while (obj) {
                ...
                // Table cells' declared line height smaller than fonts' line space can cause text overlapped if reflowed.
                if (obj->style()->height().isFixed() &&
                        !obj->isTable() && !obj->isTableCell() && !obj->isTableCol() &&
                        !obj->isTableRow() && !obj->isTableSection() ||
                        obj->isTableCell() && obj->style()->computedLineHeight() < obj->style()->font().lineSpacing()) {
                    doReflow = false;
                    break;
                }
                if (obj->isPositioned())

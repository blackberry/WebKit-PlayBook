...
public:
    ...
    int width(const TextRun& run, HashSet<const SimpleFontData*>* fallbackFonts = 0, GlyphOverflow* glyphOverflow = 0) const { return std::lroundf(floatWidth(run, fallbackFonts, glyphOverflow)); }
    ...
    int spaceWidth() const { return (int)std::ceilf(primaryFont()->adjustedSpaceWidth() + m_letterSpacing); }
    float tabWidth(const SimpleFontData& fontData) const { return 8 * std::ceilf(fontData.adjustedSpaceWidth() + letterSpacing()); }

...
float Font::floatWidthForSimpleText(const TextRun& run, GlyphBuffer* glyphBuffer, HashSet<const SimpleFontData*>* fallbackFonts, GlyphOverflow* glyphOverflow) const
{
    ...
    if (glyphOverflow) {
        glyphOverflow->top = max<int>(glyphOverflow->top, std::ceilf(-it.minGlyphBoundingBoxY()) - ascent());
        glyphOverflow->bottom = max<int>(glyphOverflow->bottom, std::ceilf(it.maxGlyphBoundingBoxY()) - descent());
        glyphOverflow->left = std::ceilf(it.firstGlyphOverflow());
        glyphOverflow->right = std::ceilf(it.lastGlyphOverflow());
    }
...
FloatRect Font::selectionRectForSimpleText(const TextRun& run, const FloatPoint& point, int h, int from, int to) const
{
    ...
    if (run.rtl()) {
        ...
        return FloatRect(point.x() + std::floorf(totalWidth - afterWidth), point.y(), std::roundf(totalWidth - beforeWidth) - std::floorf(totalWidth - afterWidth), h);
    }

    return FloatRect(point.x() + std::floorf(beforeWidth), point.y(), std::roundf(afterWidth) - std::floorf(beforeWidth), h);
}

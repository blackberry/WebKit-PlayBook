...
void InlineTextBox::paintSelection(GraphicsContext* context, int tx, int ty, RenderStyle* style, const Font& font)
{
    ...
    context->drawHighlightForText(font, TextRun(characters, length, textRenderer()->allowTabs(), textPos(), m_toAdd,
                                  ...
                                  IntPoint(m_x + tx, y + ty), h, c, style->colorSpace(), sPos, ePos, m_logicalWidth);
#else
    context->drawHighlightForText(font, TextRun(characters, length, textRenderer()->allowTabs(), textPos(), m_toAdd,
                                  !isLeftToRightDirection(), m_dirOverride || style->visuallyOrdered()),
...
void InlineTextBox::paintAttributeText(GraphicsContext* context, int tx, int ty, const DocumentMarker& marker, RenderStyle* style, const Font& font)
{
    ...
    if (backgroundColor.isValid()) {
        ...
        context->clip(IntRect(tx + m_x, ty + topOfSelectionRect, m_logicalWidth, heightOfSelectionRect));
    ...
    if (hasAttributeTextFont || backgroundColor.isValid()) {
        ...
        paintTextWithShadows(context, hasAttributeTextFont ? attributeTextFont : font, run, startPos, endPos, m_len, textOrigin, m_x + tx, m_y + ty, m_logicalWidth, logicalHeight(), 0, textStrokeWidth > 0);
        if (textStrokeWidth > 0)
            context->restore();
    }
...
void InlineTextBox::paintCaretMarker(GraphicsContext* context, int tx, int ty, const DocumentMarker& marker, RenderStyle* style, const Font& font)
{
    ...
    context->save();
    context->clip(IntRect(m_x + tx, topOfSelectionRect + ty, m_logicalWidth, heightOfSelectionRect));
    ...
    paintTextWithShadows(context, font, run, startPos, endPos, m_len, textOrigin, m_x + tx, m_y + ty, m_logicalWidth, logicalHeight(), 0, textStrokeWidth > 0);
    if (textStrokeWidth > 0)
        context->restore();
}
#endif

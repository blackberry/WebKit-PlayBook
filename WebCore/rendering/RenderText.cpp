...
typedef HashMap<RenderText*, Timer<RenderText>* > RenderTextTimerMap;
static RenderTextTimerMap* gSecureLastCharacterTimers = 0;
...
RenderText::~RenderText()
{
    ...
    if (gSecureLastCharacterTimers && m_hasSecureLastCharacterTimer) {
        Timer<RenderText>* secureLastCharacterTimer = gSecureLastCharacterTimers->get(this);
...
void RenderText::setTextInternal(PassRefPtr<StringImpl> text)
{
    ...
    if (style()) {
        ...
        case TSNONE:
            break;
        case TSCIRCLE:
            m_text.makeSecure(whiteBullet, m_shouldSecureLastCharacter);
            break;
        case TSDISC:
            m_text.makeSecure(bullet, m_shouldSecureLastCharacter);
            break;
        case TSSQUARE:
            m_text.makeSecure(blackSquare, m_shouldSecureLastCharacter);
        }
    }
...
void RenderText::momentarilyRevealLastCharacter()
{
    ...
    if (!gSecureLastCharacterTimers)
        gSecureLastCharacterTimers = new RenderTextTimerMap;

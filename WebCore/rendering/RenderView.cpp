...
static IntSize scrollOffsetForFixed(FrameView* frameView)
{
    ...
    int y = frameView->scrollPosition().y();
    if (y > frameView->maximumScrollPosition().y())
        y = frameView->maximumScrollPosition().y();
    if (y < 0)
        y = 0;
    return IntSize(0, y);
#else
    return frameView->scrollOffset();
#endif
}
...
void RenderView::mapLocalToContainer(RenderBoxModelObject* repaintContainer, bool fixed, bool /*useTransforms*/, TransformState& transformState) const
{
    ...
    if (fixed && m_frameView)
        transformState.move(scrollOffsetForFixed(m_frameView));
}

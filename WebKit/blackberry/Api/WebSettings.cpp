...
DEFINE_STATIC_LOCAL(String, BlackBerryCookieCacheEnabled, ("BlackBerryCookieCacheEnabled"));
...
WebSettings* WebSettings::standardSettings()
{
    ...
    settings.m_private->setBoolean(BlackBerryCookieCacheEnabled, true);
    settings.m_private->setDouble(BlackBerryInitialScale, -1);
    settings.m_private->setUnsigned(BlackBerryMaxPluginInstances, 1);
    settings.m_private->setInteger(BlackBerryOverZoomColor, 0xFFFFFFFF /* white */); // FIXME: Shouldn't we be using unsigned instead of int?
    ...
    settings.m_private->setTextReflowMode(BlackBerryTextReflowMode, TextReflowEnabledOnlyForBlockZoom);
...
{
    return m_private->getBoolean(BlackBerryCookieCacheEnabled);
}

void WebSettings::setIsCookieCacheEnabled(bool enable)
{
    m_private->setBoolean(BlackBerryCookieCacheEnabled, enable);
}
...
int WebSettings::maximumPagesInCache() const
{
    ...
    ASSERT(m_private->getUnsigned(WebKitMaximumPagesInCache) == WebCore::pageCache()->capacity());
...
{
    // FIXME: We shouldn't be calling into WebCore from settings. Instead the caller should perform such work.
    ...
    static HashSet<String> initGroups;
    if (path.isEmpty() || initGroups.contains(pageGroupName()))
        return;

    initGroups.add(pageGroupName());
    m_private->setString(WebKitDatabasePath, path);
    WebCore::databaseTrackerManager().initializeTracker(pageGroupName(), databasePath());
}
...
void WebSettings::setAppCachePath(const WebString& path)
{
    ...
    WebCore::cacheStorage(pageGroupName()).setCacheDirectory(appCachePath());
}
...
{
    return m_private->getInteger(BlackBerryOverZoomColor);
}

void WebSettings::setOverZoomColor(int color)
{
    m_private->setInteger(BlackBerryOverZoomColor, color);
}

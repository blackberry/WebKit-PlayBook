...
BackingStoreClient* BackingStoreClient::create(WebCore::Frame* frame, WebCore::Frame* parentFrame, WebPage* parentPage)
{
    ...
    // We do not supported nested frames for now
    if (parentFrame && parentFrame->tree()->parent())
        return 0;
...
WTF::Vector <BackingStoreClient*> BackingStoreClient::children() const
{
    ...
    for (Frame* child = m_frame->tree()->firstChild(); child; child = child->tree()->nextSibling()) {
        Olympia::WebKit::BackingStoreClient* client =
...
WebCore::IntRect BackingStoreClient::absoluteRect() const
{
    ...
    if (!isMainFrame()) {
        ...
        ASSERT(elt); // This can happen if m_frame is not from m_webPage or
                     // if the frame is in the process of loading and
                     // attaching a new document.  We might need to make this
                     // more robust, but the assert does show bugs.

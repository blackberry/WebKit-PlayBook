...
int RenderQueue::splittingFactor(const WebCore::IntRect& rect) const
{
    ...
    double renderRectArea = maxArea / pow(2.0, 0 /*default split factor*/);
...
void RenderQueue::addToRegularQueue(const WebCore::IntRect& rect)
{
    ...
    if (m_regularRenderJobsRegion.numRects() > 2)
        m_regularRenderJobsRegion = WebCore::unionRect(m_regularRenderJobsRegion.extents(), rect);
    else
...
void RenderQueue::clearRegularRenderJobs(const WebCore::IntRect& rect)
{
...
void RenderQueue::renderAllCurrentRegularRenderJobs()
{
    ...
    IntRectRegion regionNotRendered;
    if (m_parent->shouldSuppressNonVisibleRegularRenderJobs()) {
        ...
        regionNotRendered = IntRectRegion::subtractRegions(m_currentRegularRenderJobsBatchRegion, m_parent->visibleContentsRect());
    ...
    if (!m_currentRegularRenderJobsBatchRegion.isEmpty()) {
        rendered =
            m_parent->render(m_currentRegularRenderJobsBatchRegion.rects(),
                             true /*Always content only*/);
    }
    ...
    IntRect renderedRect = m_currentRegularRenderJobsBatchRegion.extents();
    m_currentRegularRenderJobsBatch.clear();
    m_currentRegularRenderJobsBatchRegion = IntRectRegion();
    m_currentRegularRenderJobsBatchUnderPressure = false;
...
void RenderQueue::renderRegularRenderJob()
{
    ...
    if (m_parent->shouldSuppressNonVisibleRegularRenderJobs()) {
        ...
        regionNotRendered = IntRectRegion::subtractRegions(rect, m_parent->visibleContentsRect());
        m_regularRenderJobsNotRenderedRegion = IntRectRegion::unionRegions(m_regularRenderJobsNotRenderedRegion, regionNotRendered);
